...
import funny.buildapp.pygerdownload.net.HttpResult
...
class MainViewModel : ViewModel() {
    ...
    companion object {
        const val appKey = "955873f76198c4d20e6478e2a9103fc8"
        const val groupKey = "b86cbab03a5c5b24022dfdfc744cfef6"
    }

    fun dispatch(action: ViewAction) {
        when (action) {
            ViewAction.Refreshing -> fetchData()
            ViewAction.StopRefreshing -> stopRefresh()
            ViewAction.FetchData -> fetchData()
        }
    }
    ...
    private fun fetchData() {
        viewModelScope.launch {
            ...
            flow {
                emit(api.appGroup(appKey, groupKey))
            }.map { it ->
                if (it.code == 0) {
                    if (it.data != null) {
